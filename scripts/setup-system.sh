...
usage() {
  cat <<EOF
Usage: scripts/setup-system.sh [--dry-run] [--system NAME] [--repo-url URL] [--git-dir PATH] [--work-tree PATH] [--sync-hardware] [--hardware-src PATH] [--hardware-dest PATH] [--skip-rebuild] [--skip-neovim-check] [--force-neovim-check] [-h|--help]

Post-install bootstrap for generic NixOS systems in this dotfiles repository.
...
Options:
  ...
  --skip-rebuild         Skip nixos-rebuild switch.
  --skip-neovim-check    Skip the post-rebuild nvim startup check.
  --force-neovim-check   Retained for callers that explicitly request the nvim startup check.
...
[[ "$SYSTEM" != "homelab" ]] || die "Use scripts/setup-homelab.sh for the staged homelab bootstrap."
...
if [[ -f "$NIXOS_FLAKE/flake.nix" ]]; then
  ...
  nix_cmd eval --no-write-lock-file --raw "$NIXOS_FLAKE#nixosConfigurations.$SYSTEM.config.networking.hostName"
elif (( DRY_RUN )); then
  nix_cmd flake show --no-write-lock-file "$NIXOS_FLAKE"
  nix_cmd eval --no-write-lock-file --raw "$NIXOS_FLAKE#nixosConfigurations.$SYSTEM.config.networking.hostName"
