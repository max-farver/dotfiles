...
Before staging a build, it also requires an executable non-service login shell, an unlocked local password, `users.mutableUsers = true`, and confirmation that the staged configuration does not declaratively manage the detected operator. These checks protect the existing physical-console login; they do not test or disclose the password.
