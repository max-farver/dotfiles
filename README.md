...
Before staging a build, it requires an executable non-service login shell, an unlocked local password, `users.mutableUsers = true`, and confirmation that the staged configuration does not declaratively manage the detected operator. After a successful build, it verifies that the staged generation itself provides that login-shell executable before it reports success. These checks protect the existing physical-console login; they do not test or disclose the password.
