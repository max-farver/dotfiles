...
-- Search navigation provided by mini.basics
...
do
  ...
  local function try_snippet_jump_forward()
    ...
    if not ok or not ms then return false end
...
-- Flash (jump/search)
map({ "n", "x", "o" }, "s", function() local ok, f = pcall(require, "flash"); if ok then f.jump() end end, { desc = "Flash" })
map({ "n", "x", "o" }, "S", function() local ok, f = pcall(require, "flash"); if ok then f.treesitter() end end, { desc = "Flash Treesitter" })
map("o", "r", function() local ok, f = pcall(require, "flash"); if ok then f.remote() end end, { desc = "Remote Flash" })
map({ "o", "x" }, "R", function() local ok, f = pcall(require, "flash"); if ok then f.treesitter_search() end end, { desc = "Treesitter Search" })
map("c", "<c-s>", function() local ok, f = pcall(require, "flash"); if ok then f.toggle() end end, { desc = "Toggle Flash Search" })
...
-- Grug-Far
map("n", "<leader>sg", function() local ok, g = pcall(require, "grug-far"); if ok then g.open() end end, { desc = "Search (grug-far)" })
...
-- Debugmaster toggle
map("n", "<leader>dd", function() local ok, dm = pcall(require, "debugmaster"); if ok then dm.mode.toggle() end end, { desc = "Toggle Debug Mode" })
...
vim.api.nvim_create_autocmd("User", {
  pattern = "VeryLazy",
  callback = function()
    -- group hints handled by mini.clue
    local ok, Snacks = pcall(require, "snacks")
...
do
  ...
  map("n", "<leader>gD", function() with_gitsigns("diffthis")("~") end, { desc = "Diff This (~)" })
